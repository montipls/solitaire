if __name__ == 'game.interaction':
    ...
    def winscreen_events():
        # only runs on button hover
        if events.lmbd:
            game.won = False
            game.deck = []
            ...
            game.pull = []
            scene.columns = scene.generate_columns(scene.COLUMN_COUNT)
            scene.deck = scene.generate_deck()
            game.stacks = [
                [],
                [],
                [],
